...
install() {
    bash <(curl -Ls "$(gh_url "https://raw.githubusercontent.com/hxzl666/serv00-sui/main/install.sh")")
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="$(gh_url "https://raw.githubusercontent.com/hxzl666/serv00-sui/main/install.sh")"
...
update_shell() {
    wget -O ${BIN_DIR}/s-ui -N --no-check-certificate "$(gh_url "https://raw.githubusercontent.com/hxzl666/serv00-sui/main/s-ui.sh")"
    if [[ $? != 0 ]]; then
        echo ""
